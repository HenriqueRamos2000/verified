...
const protectedMinhaMensagem = (request, response) => {
  const token = request.headers["authorization"];
  ...
  try {
    ...
    response.json({ message: "Estou Dentro!" });
  } catch (error) {
    return response.status(403).json({ message: "Token inválido ou expirado" });
  }
};

const protectedTimestamp = (request, response) => {
  const token = request.headers["authorization"];
  ...
  try {
    ...
    const string = timestamp.toLocaleString("pt-BR");
...
module.exports = { login, protectedContent, protectedMinhaMensagem, protectedTimestamp };
